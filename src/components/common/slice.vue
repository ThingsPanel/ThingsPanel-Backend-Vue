<template>
  ...
  >
    ...
    >
        <span class="card-title" v-if="item.chart_type !== 'x_number'">{{ item.title }}</span>
        <div class="right-buttons" v-show="isshowBtns">
            <i class="ml-2 fa flaticon-delete text-success pointer" @click="delpanel(item.id)" ></i>
            <i class="ml-2 fa fa-cog text-success pointer" @click="setupCon(item.id, item.title)" ></i>
            <LegendBtn 
                class="x-btn" 
      ...
      >
        <v-card class="card">
            ...
            <v-card-text>
                <v-container>
                    <v-row>
                        <v-text-field label="Id" v-show="false" />
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn class="canclebtn" text @click="close">{{ $t("COMMON.CANCEL") }}</v-btn>
                <v-btn class="confbtn" text @click="onSubmit">{{ $t("COMMON.SAVE") }}</v-btn>
            </v-card-actions>
        </v-card>
      </v-form>
    </v-dialog>
    ...
    methods: {
        ...
        handleWebsocketData() {
            ...
            try {
                ...
                console.log(socketData, this.item.id);
