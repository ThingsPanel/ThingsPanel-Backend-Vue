<template>
	<div class="dashboard" ref="dashboard" v-loading.fullscreen="false" element-loading-text="Loading..." data-app="true">
		<grid-layout 
            :layout="slices" 
            :col-num="gridColNum" :row-height="gridRowHeight" 
	...
	export default {
		...
		methods: {
			...
			getTime() {
				...
				return parseInt(data.getFullYear() + month + date + hour + minute + second);
			},
			...
			calPopPosition(expandTarget) {
				...
				const top = Math.round((winH - height) / 2) -
			...
			resizedEvent(i, newH, newW, newHPx, newWPx) {
				...
				this.slices = this.slices.map((item) => {
					...
					var id = item.id;
					var config = '{"x":' + item.x + ',"y":' +
                                item.y + ',"w":' + item.w + ',"h":' + 
                                item.h + ',"width":' + item.width + ',"height":' + item.height +
                            "}";
			...
			movedEvent(i, newX, newY) {
				...
				this.slices = this.slices.map((item) => {
					...
					var id = item.id;
					var config = '{"x":' + item.x + ',"y":' + item.y + ',"w":' + item.w +
						            ',"h":' + item.h + ',"width":' + item.width + 
                                    ',"height":' + item.height +
						        "}";
